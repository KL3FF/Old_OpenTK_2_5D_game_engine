...
namespace Com.Engine
{
    public static class ModelHandler
    {
        ...
        public static void DrawGui(Matrix4 view, Matrix4 projection, float x, float y, float width, float height, string shader, string texture, Vector2 texStart, Vector2 texEnd)
        {
            // Überprüfen, ob der Shader gewechselt hat
            if ((lastShader != shader) || (lastShader == ""))
            {
            ...
            // Überprüfen, ob die Textur gewechselt hat
            if ((lastTexture != texture) || (lastTexture == ""))
            {
                if (lastTexture != "")
                {
                    TextureHandler.Unbind(lastTexture);
            ...
            // Transformationen für das Modell (Position und Größe)
            Matrix4 model = Matrix4.Identity;
            Matrix4 translation = Matrix4.CreateTranslation(x, y, 0f);  // Position im 2D-Raum
            Matrix4 scaling = Matrix4.CreateScale(width, height, 0f);   // Größe des GUI-Elements

            model *= translation * scaling; // Zuerst Translation, dann Skalierung
